...
**[refactor] Arch-1-H — Assess and remove `calc_internal.h`** — Now that the four `extern` globals are gone, `calc_internal.h`'s only remaining value is re-exporting other headers. Audit each of the 11 callers; replace `#include "calc_internal.h"` with targeted includes of what each file actually needs; delete `calc_internal.h`. Complexity decrease. Full context: [docs/ARCHITECTURE_REVIEW_2026-05-01.md](docs/ARCHITECTURE_REVIEW_2026-05-01.md) item 1-H.

**[refactor] Graph_HandleKey() unified entry point** — Deepening Opportunity 1 from the 2026-04-30 architecture review, not addressed by the T/F items. Currently tracing what happens on any graph-mode keypress requires bouncing across four files (`graph.c`, `graph_ui.c`, `graph_ui_range.c`, `graph_draw.c`). T2-C already moved the cursor states (`s_trace`, `s_free`, `s_zbox`) into `graph.c`; this item consolidates the dispatch logic to match. Add a single `Graph_HandleKey(token)` entry point in `graph.c` that owns the full round-trip: read current graph mode → dispatch to cursor/mode handler → re-render if needed. `graph_ui.c` becomes a thin caller rather than the primary dispatch site. Files: [App/Src/graph.c](App/Src/graph.c), [App/Src/graph_ui.c](App/Src/graph_ui.c), [App/Inc/graph.h](App/Inc/graph.h). *Arch-2 circular include chain is now broken (2026-05-04) — the Arch-2 prerequisite for this item is satisfied. Unlocks cleaner implementations of ZOOM cursor-pick, `Input` no-arg, TRACE auto-pan, and Interactive DRAW below.*
