...
**[complexity] P29 — stat renderer extraction from `graph.c`** — During the DRAW refactor, the draw layer moved to [App/Src/graph_draw.c](App/Src/graph_draw.c) and `graph_render_setup()` was exposed as a hook point. Remaining in `graph.c`: the stat renderer functions (`Graph_DrawScatter`, `Graph_DrawXYLine`, `Graph_DrawHistogram`, `stat_plot_prepare`, `draw_line_px`, ~150 lines) that are tightly coupled to private canvas state. Assess whether extraction to a `graph_stat.c` is worth the coupling surface it would add, then either extract or document the decision to leave them in place. File: [App/Src/graph.c](App/Src/graph.c).
