...
**[refactor] Arch-3 — Generic Menu Screen module for PRGM submenus** — `ui_prgm_ctl.c`, `ui_prgm_io.c`, `ui_prgm_exec.c`, and `ui_prgm_mode.c` are ~80% copy-pasted boilerplate (tab/cursor/scroll/LVGL). Extract a `MenuScreen_t` module that takes a descriptor and owns all navigation logic; rewrite each submenu as a ~40-line descriptor + callback. The new module should own a `MenuState_t` internally — do not duplicate list-navigation. Full task list: [docs/ARCHITECTURE_REVIEW_2026-05-01.md](docs/ARCHITECTURE_REVIEW_2026-05-01.md) Opportunity 3.
...
**TRACE auto-pan at viewport edge** — Guidebook p. 3-10: when the trace cursor reaches `Xmin` or `Xmax`, the viewing window pans by `(Xmax−Xmin)/2` and `Xmin`/`Xmax` update to reflect the new position. `handle_trace_mode()` in `graph_ui.c` currently clamps at the boundary instead. Add a pan branch that shifts both bounds and triggers a re-render. File: [App/Src/graph_ui.c](App/Src/graph_ui.c). *Can be done before the Graph_HandleKey refactor (Large section below) but becomes a single-file change in `graph.c` if that refactor runs first.*
...
**[refactor] Arch-1 — Encapsulate expression editor shared mutable state** — `calc_internal.h` exposes four `extern` globals (`insert_mode`, `cursor_visible`, `sto_pending`, `expr`) that are writable by all 11 translation units that include it. Make them `static` in `calculator_core.c`; add getter/setter functions to `calculator_core.h`; update all 11 callers. This is the highest-leverage architectural change: it breaks the super-module coupling, makes each UI module independently compilable, and sharpens the expression-editor seam. *Recommended before Arch-2 and Arch-3 since those refactors benefit from the reduced coupling surface.* Full task list and gotchas (STO synthesis, expr pointer semantics): [docs/ARCHITECTURE_REVIEW_2026-05-01.md](docs/ARCHITECTURE_REVIEW_2026-05-01.md) Opportunity 1.

**[refactor] Graph_HandleKey() unified entry point** — Deepening Opportunity 1 from the 2026-04-30 architecture review, not addressed by the T/F items. Currently tracing what happens on any graph-mode keypress requires bouncing across four files (`graph.c`, `graph_ui.c`, `graph_ui_range.c`, `graph_draw.c`). T2-C already moved the cursor states (`s_trace`, `s_free`, `s_zbox`) into `graph.c`; this item consolidates the dispatch logic to match. Add a single `Graph_HandleKey(token)` entry point in `graph.c` that owns the full round-trip: read current graph mode → dispatch to cursor/mode handler → re-render if needed. `graph_ui.c` becomes a thin caller rather than the primary dispatch site. Files: [App/Src/graph.c](App/Src/graph.c), [App/Src/graph_ui.c](App/Src/graph_ui.c), [App/Inc/graph.h](App/Inc/graph.h). *Unlocks cleaner implementations of ZOOM cursor-pick, `Input` no-arg, TRACE auto-pan, and Interactive DRAW below.*

**[refactor] PersistBlock_t sub-struct adoption** — Introduce `GraphPersist_t`, `StatPersist_t`, `MatrixPersist_t`, `PrgmPersist_t`, `ModePersist_t` sub-structs; add per-section version fields; replace the monolithic migration switch with per-section switches. Must be adopted atomically at the next feature that bumps `PERSIST_VERSION` — do not adopt piecemeal. Design documented in `docs/TECHNICAL.md` "Persist Migration Design". Files: [App/Inc/app_common.h](App/Inc/app_common.h), [App/Src/persist.c](App/Src/persist.c), and every module that reads/writes `PersistBlock_t`. No prerequisites; deferred until the next `PERSIST_VERSION` bump makes the migration unavoidable.

**ZOOM In/Out/Integer cursor-pick mode** — Guidebook pp. 3-13/3-14/3-16: ZOOM In, Out, and Integer require the user to position a crosshair cursor on the graph before ENTER commits; the selected point becomes the new window centre. Currently all three apply their zoom factor immediately without cursor interaction. Requires a cursor-pick state machine (similar to ZBox but single-point) wired into the ZOOM dispatch flow; all three modes share the same infrastructure and should be implemented together. Files: [App/Src/ui_graph_zoom.c](App/Src/ui_graph_zoom.c), [App/Src/graph_ui.c](App/Src/graph_ui.c). *Recommended after Graph_HandleKey refactor. Hard prerequisite for Interactive on-graph DRAW.*
...
**Interactive on-graph DRAW** — Guidebook pp. 5-2, 5-5/5-6: from a visible graph, `2nd+DRAW` → select `Line(`/`PT-On(`/`PT-Off(`/`PT-Chg(` → move a crosshair cursor to the first point (ENTER), then the second (ENTER) — all done visually on the canvas. Currently all DRAW commands require pre-typed coordinate argument lists in the expression buffer. Implement: after selecting a compatible DRAW command, enter the ZOOM cursor-pick state machine, collect 1–2 graph coordinates, then call the existing draw function with those coordinates. Files: [App/Src/graph_ui.c](App/Src/graph_ui.c), [App/Src/ui_draw.c](App/Src/ui_draw.c). *Hard prerequisite: ZOOM cursor-pick mode. Recommended after Graph_HandleKey refactor.*
