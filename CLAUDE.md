...
**[complexity] P29 — DRAW menu complexity follow-up (partially resolved)** — Draw layer extracted to `graph_draw.c`; `Graph_DrawTrace()` split into `graph_draw_trace_param/func` static helpers (118→17 lines); `graph_render_setup()` deduplicated from both render functions; `GRAPH_BUF_ADDR` named constant added. Remaining: (1) stat renderer functions (`Graph_DrawScatter/XYLine/Histogram`, `stat_plot_prepare`, `draw_line_px`) still in `graph.c` (~150 lines, tightly coupled to private canvas state — `graph_render_setup()` now exposed as a hook point if extraction to `graph_stat.c` is pursued); (2) `try_execute_draw_command` was moved to `ui_draw.c` in Phase 3. P29 remaining debt: stat renderer extraction only. Assess at next code-organisation review.
...
**P33 — MODE screen unimplemented rows** — Rows 1/4/5/6/7/8 complete. Row 1 (Normal/Sci/Eng notation) is the only remaining unwired row. Spec: `docs/MENU_SPECS.md` lines 25–43.
...
**P37 — PRGM Connected/Dot graph mode commands** — The TI-81 guidebook (p. 8-16) lists `Connected` and `Dot` as PRGM [MODE] GRAPH sub-menu commands that set graph draw style during program execution. Analogous to P36 (Norm/Sci/Eng). Implementation: add `TOKEN_CONNECTED` and `TOKEN_DOT` tokens; register in PRGM MODE GRAPH sub-menu; handle in `prgm_exec.c` by calling `Graph_SetConnectedMode()`. Prerequisite: `Graph_SetConnectedMode()` is now implemented. Files: `App/Inc/app_common.h` (tokens), `App/Src/ui_prgm.c` (menu), `App/Src/prgm_exec.c` (executor).

**P39 — PRGM Sequence/Simul graph mode commands** — The TI-81 guidebook (p. 8-16) lists `Sequence` and `Simul` as PRGM [MODE] GRAPH sub-menu commands that set graph draw order during program execution. Analogous to P37 (Connected/Dot). Implementation: add `TOKEN_SEQUENCE` and `TOKEN_SIMUL` tokens; register in PRGM MODE GRAPH sub-menu; handle in `prgm_exec.c` by calling `Graph_SetSequentialMode()`. Prerequisite: `Graph_SetSequentialMode()` is now implemented (P38). Files: `App/Inc/app_common.h` (tokens), `App/Src/ui_prgm.c` (menu), `App/Src/prgm_exec.c` (executor).
