...
| Area | Est. Done | Notes |
|---|---|---|
| MATRIX | ~85% | Variable dimensions 1–6×6 per matrix; scrolling cell editor with dim mode; `det(` and `T` ops in MATRX menu; row operations (RowSwap/Row+/*Row/*Row+) fully implemented in MATRX MATH menu and engine; arithmetic (+, −, ×, scalar×matrix) fully evaluated; `det(ANS)` / `[A]+ANS` chains work; persist across power-off; `[A]`/`[B]`/`[C]` cursor/DEL atomicity fixed; matrix tokens blocked in Y= editor. **Known gaps (surfaced 2026-05-18):** matrix inversion (`[C]^-1`), matrix squaring (`[B]^2`), matrix negation (`-(−)[C]`), and element read `[A](r,c)` as expression operand are all unimplemented. Pending: hardware validation. |
...
#### Guidebook compliance — surfaced by 2026-05-18 annotated-guidebook review

Items are ordered easiest/highest-impact first within each tier. Source: `docs/Datasheets/TI81Guidebook copy.md` IMPL-NOTE annotations.

**[bug] DRAW layer not cleared on graph replot (undocumented Smart Graph divergence)** — Guidebook p. 5-3: TI-81 erases all DRAW content whenever the graph is replotted. `Graph_Render()` (`App/Src/graph.c:574`) calls `Graph_ApplyDrawLayer` blending existing draw content on top after every render; `Graph_DrawLayerClear()` is never called automatically. DRAW content survives Y= edits, RANGE changes, and ZOOM operations. Fix: call `Graph_DrawLayerClear()` at the top of `Graph_Render()` before the equation rendering loop (~line 408). If retaining draw content across replots is intentional, document it in the "Deliberate Deviations" table instead. Files: `App/Src/graph.c`.

**[bug] RANGE screen accepts inverted ranges (Xmin ≥ Xmax) without error** — Guidebook: Xmin < Xmax and Ymin < Ymax must hold or ERROR 11 RANGE fires at [GRAPH]. `Graph_SetWindow` accepts inverted ranges silently; `graph_coord_math_x_to_px` divides by a negative value. Add validation at commit time in `App/Src/graph_ui_range.c` and/or at the top of `Graph_Render()`, returning `CALC_ERR_RANGE`. Files: `App/Src/graph_ui_range.c`, `App/Src/graph.c`.

**[bug] 1-Var statistics ignores Y-as-frequency** — Guidebook p. 7-3, p. 7-7: in one-variable data, each Y value is the frequency (repeat count) of its associated X value. `CalcStat_Compute1Var` (`App/Src/calc_stat.c:61–93`) ignores `list_y` and treats each `list_x[i]` as a single data point, producing wrong n, Σx, mean, and standard deviations for weighted data (e.g. the p. 7-17 worked example). Fix: weight each point by `list_y[i]` as an integer frequency; effective n = Σy[i], Σx = Σ(x[i]·y[i]), Σx² = Σ(x[i]²·y[i]). Guard against zero/negative frequencies. Update associated tests. Files: `App/Src/calc_stat.c`, `App/Tests/test_stat.c`.

**[bug] LinReg/LnReg/ExpReg/PwrReg regression a/b coefficient convention is swapped vs guidebook** — Guidebook p. 7-9: linear model is `y = a + bX` where `a` is the intercept and `b` is the slope. `CalcStat_ComputeLinReg` (`App/Src/calc_stat.c:95–110`) assigns `reg_a = slope`, `reg_b = intercept` — opposite convention, propagating to `calc_variables[0]`/`[1]` (A and B) and VARS LR display. Same swap applies to LnReg, ExpReg, PwrReg per comments in `calc_stat.c:127`. Users accessing `a`/`b` from VARS LR get swapped values vs guidebook. Files: `App/Src/calc_stat.c`, `App/Tests/test_stat.c`.

**[bug] RegEQ always emits `aX+b` linear form regardless of regression model** — `vars_format_value` case 2 item 3 in `App/Src/ui_vars.c:135` always generates `"%sX+%s"`. Correct forms: LinR=`aX+b`, LnR=`a+b*ln(X)`, ExpR=`a*b^X`, PwrR=`a*X^b`. Fix: add `uint8_t last_model` field to `StatResults_t` (`App/Inc/app_common.h`); set it in each `CalcStat_Compute*` function; switch on it in `vars_format_value` to emit the correct string. Files: `App/Inc/app_common.h`, `App/Src/calc_stat.c`, `App/Src/ui_vars.c`.

**[bug] Histogram uses fixed 10 bins instead of guidebook-specified Xscl width** — `Graph_DrawHistogram` (`App/Src/graph.c:1232–1291`) ignores `graph_state.x_scl` and divides x-range into `HIST_BINS=10` fixed bins. Fix: use `bin_width = x_scl`; compute `first_bin_left = floor(x_min / x_scl) * x_scl`; accumulate `floor((x[i] - first_bin_left) / x_scl)`. Also fix boundary rule: guidebook p. 7-15 says data on a bar edge goes into the bar to the right; current `(int)((x - x_min) / bin_width)` places boundary values in the left bar. Files: `App/Src/graph.c`.

**[bug] nDeriv( ΔX argument is consumed but silently ignored** — `rpn_eval_special()` at `App/Src/calc_engine.c:1334` hardcodes `eps = 1e-3f`; the user-supplied second argument is popped from the stack but not used. Guidebook says the second argument controls the differentiation step. Fix: read the second argument from the stack instead of using the hardcode. Files: `App/Src/calc_engine.c`.

**Matrix inversion, squaring, and negation not implemented** — Three related gaps in matrix operator support confirmed by guidebook Ch. 6: (1) `[C]^-1` (matrix inversion) — `MATH_OP_POW` routes through scalar `powf`; no Gauss-Jordan or Cramer's rule. (2) `[B]^2` (matrix multiply M×M) — same scalar `powf` path. (3) `-(−)[C]` (element-wise negation) — `MATH_OP_NEG` negates the slot-index integer rather than the elements. All three require adding matrix-aware branches to the relevant `eval_*` functions in `App/Src/calc_engine.c`. Inversion is the most complex; squaring and negation are simpler. Files: `App/Src/calc_engine.c`.

**Matrix element read `[A](r,c)` as expression operand not implemented** — Guidebook p. 6-10: `[A](row,col)` in an expression returns a scalar. Tokenizer (`App/Src/calc_engine.c:260–272`) recognises `[A]`/`[B]`/`[C]` as whole-matrix references only; `(` after a matrix token is treated as arithmetic grouping. Requires a new tokenizer path that detects `MATH_MATRIX_VAL` followed by `(`, peeks for `int,int)`, and resolves to the element scalar. Files: `App/Src/calc_engine.c`.

**`{x}(n)` / `{y}(n)` STO write path not implemented** — Guidebook p. 7-16: `value STO→ {x}(n)` and `value STO→ {y}(n)` write back into `stat_data.list_x[n-1]` / `list_y[n-1]`. Read access via registered callbacks is implemented; no STO routing detects a `{x}(n)` target. Fix: detect the pattern in the STO handler (`App/Src/ui_sto.c` or `App/Src/calculator_core.c`) and write to `stat_data.list_x[index-1]`. Files: `App/Src/ui_sto.c`, `App/Src/calc_stat.c`.

**[testing] PwrReg has no positive-case host test** — `test_stat.c` has `test_linreg_perfect`, `test_lnreg`, `test_expreg` but no `test_pwrreg` success case. Add a test using a known power-law dataset (e.g. `y = 2x^3`). File: `App/Tests/test_stat.c`.
