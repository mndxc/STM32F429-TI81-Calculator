...
| 2026-04-25 | Code organisation | B | B | Parametric Y-VARS tabs: Y tab 4→10 items, ON/OFF 5→8 items; scroll indicators added; `yvars_do_enable` handles param pairs; digit shortcuts extended; mirrors VARS/ZOOM scrolling pattern; complexity delta: neutral |
| 2026-04-26 | Code organisation | B | B | `Dim{x}` VARS DIM tab item 7: `vars_dim_names` expanded to 7 entries; `vars_tab_item_count[3]` 6→7; `vars_format_value` case 6 reads `Stat_GetData()->list_len`; `MENU_SPECS.md` updated; complexity delta: neutral |
| 2026-04-26 | Code organisation | B | B | RESET menu (2nd++): `MODE_RESET_CONFIRM`; `ui_reset.c`/`.h` (MEMORY screen + handler); `Persist_Reset()` in `persist.c`; `k_mode_handlers[]` 25→26; `hide_all_screens()` extended; complexity delta: neutral |
| 2026-04-26 | Testing | A | A | `Shade(` full argument set: `Graph_Shade` upgraded from 2-scalar-constants to `(lowerfunc, upperfunc [, resolution, Xbeg, Xend])`; per-column `Calc_PrepareGraphEquation`+`Calc_EvalGraphEquation`; boundary curves drawn; resolution column-density; Xbeg/Xend clip region; `shade_extract_str` depth-aware parser; 15 new host assertions (group 5e); suite grows to 10 suites / 846 assertions; complexity delta: neutral |
...
Items are ordered within each tier by estimated difficulty (easiest first). Dependencies between items are noted inline.
...
**Interactive on-graph DRAW** — Guidebook pp. 5-2, 5-5/5-6: from a visible graph, `2nd+DRAW` → select `Line(`/`PT-On`/`PT-Off`/`PT-Chg` → move cursor to first point (ENTER), second point (ENTER) — all done visually on the graph canvas. Currently all DRAW commands require pre-typed argument lists in the expression buffer. Implement: after selecting a compatible DRAW command, enter a cursor-pick state (reusing ZOOM cursor-pick infrastructure), collect 1–2 graph coordinates, then call the existing draw function. Files: `App/Src/graph_ui.c`, `App/Src/ui_draw.c`. *Prerequisite: ZOOM cursor-pick mode (above).*

**Error display format and Goto Error** — Guidebook pp. 1-26, B-4: errors displayed as `ERROR nn type` (19 numbered codes with type labels MATH/SYNTAX/MEMORY/RANGE/ZOOM/BREAK/PRGM/INVALID); error screen offers `<Goto Error>` (re-enters expression editor with cursor at the fault offset) and `<Quit>` (clears to Home). Current implementation uses ad-hoc strings with no codes and no navigation. Requires: (1) map `CalcError_t` codes to TI-81 numbered strings; (2) new error-display screen/mode; (3) Goto Error state that re-opens the expression editor with cursor at the error byte offset returned by `Calc_Evaluate()`. Files: `App/Src/calculator_core.c`, `App/Src/ui_input.c`, `App/Src/calc_engine.c`.

**[refactor] PersistBlock_t sub-struct adoption** — Introduce `GraphPersist_t`, `StatPersist_t`, `MatrixPersist_t`, `PrgmPersist_t`, `ModePersist_t` sub-structs; add per-section version fields; replace the monolithic migration switch with per-section switches. Must be adopted atomically at the next feature that bumps `PERSIST_VERSION`. Design documented in `docs/TECHNICAL.md` "Persist Migration Design". Do not adopt piecemeal. Files: `App/Inc/app_common.h`, `App/Src/persist.c`, and every module that reads/writes `PersistBlock_t`.
...
**[hardware] Free-cursor + TRACE toggle hardware validation** — Implementation complete (2026-04-24). Validate on hardware: (1) Press GRAPH — verify a blinking white crosshair appears at screen centre; X= and Y= show current math coords; (2) Arrow keys move crosshair one pixel step at a time; X=/Y= update with each press; cursor stays visible immediately after each key (blink resets); (3) Press TRACE from free-cursor position — verify trace cursor snaps to the nearest curve at the current X (green crosshair replaces white); (4) In trace, press TRACE again — verify cursor snaps back to x_mid on the same equation (does NOT exit trace); (5) In trace, press GRAPH — verify graph redraws with blinking free cursor at centre; (6) In trace, press a digit key — verify trace exits and digit appears on home screen; (7) From Y=, RANGE, ZOOM menus — press GRAPH — verify blinking free cursor appears; (8) Confirm free cursor timer stops (no blinking) whenever the home screen or any menu is shown; (9) confirm free cursor visible in parametric mode. Files: `App/Src/graph_ui.c`, `App/Src/graph.c`.

**[hardware] P33h — Connected/Dot mode hardware validation** — P33 implementation complete, 10/10 host tests pass. Validate on hardware: (1) MODE screen Row 5 shows "Connected | Dot"; navigate with LEFT/RIGHT, ENTER commits; (2) select Dot, graph `Y1=sin(X)` — verify only individual pixels are plotted with no connecting lines; (3) select Connected, regraph — verify interpolated line segments return; (4) confirm Row 4 (Func/Param) still works after the row-index bug fix — select Param, verify Y= switches to X₁t/Y₁t layout; (5) power off and on — confirm Connected/Dot selection persists. Files: `App/Src/graph.c`, `App/Src/ui_mode.c`.
...
**[hardware] P38h — Sequential/Simultaneous hardware validation** — P38 implementation complete, 3/3 new host assertions pass (total 742). Validate on hardware: (1) MODE screen Row 6 shows "Sequential | Simul"; navigate with LEFT/RIGHT, ENTER commits; (2) enter two equations (Y1=X, Y2=X²) with window X[-5,5] Y[-5,25]; select Simul — verify both curves advance across the screen together (visible interleaving as graph draws); (3) switch back to Sequential — verify Y1 plots fully before Y2 starts; (4) parametric mode: enter X₁t=cos(T), Y₁t=sin(T) and X₂t=2cos(T), Y₂t=2sin(T); Simul draws both circles advancing together, Sequential draws the inner circle then the outer; (5) power off and on — confirm Sequential/Simul selection persists. Files: `App/Src/graph.c`, `App/Src/ui_mode.c`.

**[hardware] P40h — Polar coordinate display hardware validation** — P40 implementation complete. Validate on hardware: (1) MODE screen Row 8 shows "Rect | Pol"; navigate with LEFT/RIGHT, ENTER commits; (2) select Pol, open graph with Y1=X²; move free cursor — verify readout shows R= and θ= instead of X= and Y=; (3) press TRACE, move along curve — verify R=/θ= readout in trace mode; (4) parametric mode: enter X₁t=cos(T), Y₁t=sin(T); TRACE shows R=/θ=/T= readout; (5) switch back to Rect — verify readout returns to X=/Y=; (6) power off and on — confirm Pol selection persists; (7) home screen: type `R>P(-1,0)` ENTER — result is 1; type θ ENTER — result is 3.141... (rad mode) or 180 (deg mode); (8) type `P>R(1,0)` ENTER — result is 1; type Y ENTER — result is 0; (9) type `3` STO θ ENTER — verify θ variable stores 3; type θ ENTER — result is 3. Files: `App/Src/graph.c`, `App/Src/ui_mode.c`, `App/Src/calc_engine.c`, `App/Src/ui_input.c`.

**[hardware] P29h — DRAW menu hardware validation** — P29 implementation complete, build clean, host tests pass. Validate on hardware: (1) `2nd+PRGM` opens DRAW menu with 7 items; digit shortcuts 1–7 work; UP/DOWN navigation works; CLEAR exits; (2) `ClrDraw` entered from expression buffer clears draw layer and shows "Done"; (3) `Line(0,0,5,5)` draws a diagonal line on the graph canvas; (4) `PT-On(2,3)` sets a pixel; `PT-Off(2,3)` clears it; `PT-Chg(2,3)` toggles it; (5) `DrawF sin(X)` draws the sine curve as a white overlay; (6) `Shade(-1,1)` shades between constant boundaries y=−1 and y=1; `Shade(X+1,X^3-8*X)` shades where X+1 < X^3−8X (guidebook p. 5-10 example); `Shade(sin(X),cos(X),2,-3,3)` shades with resolution=2 and X boundaries; (7) draw layer persists across GRAPH re-renders (e.g. ZOOM then return — drawn content remains); (8) `ClrDraw` clears all drawn content. Files: `App/Src/ui_draw.c`, `App/Src/graph.c`, `App/Src/calculator_core.c`.
...
15. **FLASH sector map — firmware has grown into FLASH_SECTOR_10** — On STM32F429ZIT6 (2MB, 12 sectors per bank), the sector layout is: sectors 0–3 = 16 KB, sector 4 = 64 KB, sectors 5–11 = 128 KB. `FLASH_SECTOR_7` is at **0x08060000**. As of 2026-04-28 the firmware is ~820 KB, which extends ~33 KB into `FLASH_SECTOR_10` (0x080C0000). The persist block has therefore been moved to **`FLASH_SECTOR_11` at 0x080E0000**; the linker FLASH region is capped at 896 KB so any future overflow is a link error rather than a silent persist collision. If firmware ever exceeds 896 KB, move persist to a Bank 2 sector and update `PERSIST_SECTOR`/`PERSIST_FLASH_ADDR` in `persist.h` and the linker cap in `STM32F429XX_FLASH.ld`.
