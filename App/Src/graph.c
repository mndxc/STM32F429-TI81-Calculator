...
/* DRAW menu overlay layer — SDRAM after graph_buf_clean + 64 KB LVGL heap gap.
 * 0x0000 = transparent sentinel; any other value = drawn pixel color.
 * Layout: graph_buf=0xD0025800 (150 KB), graph_buf_clean=0xD004B000 (150 KB),
 *         LVGL heap=0xD0070800 (64 KB), draw_buf=0xD0080800 (150 KB). */
...
/**
 * @brief Blends the user draw layer over graph_buf.
 *        Called at the end of every render pass so that drawn objects persist
 *        across equation re-renders (ClrDraw resets draw_buf to zero).
 */
static void apply_draw_layer(void)
{
    for (int32_t i = 0; i < GRAPH_W * GRAPH_H; i++) {
        if (draw_buf[i] != 0)
            graph_buf[i] = draw_buf[i];
    }
}
...
void Graph_Shade(float y_low, float y_high, uint16_t fill_color)
{
    ...
    for (int32_t px = 0; px < GRAPH_W; px++) {
        int32_t py_top = math_y_to_px(y_high);
        int32_t py_bot = math_y_to_px(y_low);
