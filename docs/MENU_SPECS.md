...
Five tabs: XY / Σ / LR / DIM / RNG. Tab LEFT/RIGHT; item UP/DOWN or number key; ENTER inserts the current numeric value of the selected variable at the cursor. DIM tab has 7 items (Arow–Ccol, Dim{x}); RNG tab has 10 items with scroll. ȳ/Sy/σy/Σy/Σy²/Σxy computed on-the-fly from `stat_data`; RegEQ inserts `aX+b` from `stat_results.reg_a/reg_b`; Dim{x} reads `Stat_GetData()->list_len`.
